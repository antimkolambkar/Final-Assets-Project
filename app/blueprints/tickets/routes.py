...
def index():
    ...
    # IT Engineers only see their assigned tickets
    # unless Super Admin / IT Admin
    if current_user.role == Role.IT_ENGINEER:
        ...
        query = query.filter_by(
            assigned_engineer_id=engineer_filter
        )
...
def create_ticket():
    ...
    if not all([
        ...
        flash(
            'Subject, description, and employee selection are required.',
            'danger'
        )

        return redirect(
            url_for('tickets.index')
        )
...
@tickets_bp.route(
    '/api/ai/correct-description',
    methods=['POST']
)
@login_required
def ai_correct_description():
    ...
    try:
        ...
        response = client.responses.create(
            model='gpt-5.6-luna',
            instructions=(
                'You are an IT helpdesk writing assistant. '
                'Correct spelling, grammar, punctuation, and '
                'sentence structure. '
                'Make the description clear and professional '
                'for an internal IT helpdesk ticket. '
                'Keep the original meaning and all technical details. '
                'Do not invent information. '
                'Do not remove important information. '
                'Do not change product names, error messages, '
                'asset names, employee names, or technical terms. '
        ...
        if not corrected:
            ...
            return jsonify({
                'error': (
                    'AI did not return a corrected description.'
                )
            }), 500
        ...
        current_app.logger.exception(
            'AI description correction failed.'
        )

        return jsonify({
            'error': (
                'Unable to correct the description right now. '
                'Please try again.'
            )
        }), 500
...
def detail(ticket_id):
    ...
    # IT Engineers can only view assigned tickets
    if (
        current_user.role == Role.IT_ENGINEER
        and ticket.assigned_engineer_id != current_user.id
    ):

        flash(
            'Permission denied. You can only view tickets assigned to you.',
            'danger'
        )
    ...
    assigned_assets = Asset.query.filter_by(
        assigned_employee_id=ticket.employee_id
    ).all()
...
def assign_ticket(ticket_id):
    ...
    if not current_user.can_assign_tickets():
        ...
        flash(
            'Permission denied. Only IT Administrators can assign tickets.',
            'danger'
        )
    ...
    success, msg = TicketService.assign_engineer(
        ticket_id,
        engineer_id,
...
def update_status(ticket_id):
    ...
    success, msg = TicketService.update_status(
        ticket_id,
        new_status,
        current_user.full_name
    )
...
def download_attachment(attachment_id):
    ...
    attachment = TicketAttachment.query.get_or_404(
        attachment_id
    )
